...
void MA_ABE::Decrypt(element_t res, std::vector<MA_ABE_skgidA *> &skgidAs, MA_ABE_ciphertext &C){
    ...
    for(int i=0;i<rows;i++){
        ...
        for(int k=0;k<skgidAs.size();k++){
            if(skgidAs[k]->getA() == pai[i]){
                element_set(tmp_GT, C.getCi(i)[ci_1]);
                // e(skgidA_0,ci_2)
                element_pairing(tmp_GT_2, skgidAs[k]->get(skgidA_0), C.getCi(i)[ci_2]);
                element_mul(tmp_GT, tmp_GT, tmp_GT_2);
                // e(HGID(0,gid),ci_3)
                this->HGID(tmp_G, 0, skgidAs[k]->getGid());
                element_pairing(tmp_GT_2, tmp_G, C.getCi(i)[ci_3]);
                element_mul(tmp_GT, tmp_GT, tmp_GT_2);
                // e(skgidA_1,ci_4)
                element_pairing(tmp_GT_2, skgidAs[k]->get(skgidA_1), C.getCi(i)[ci_4]);
                element_mul(tmp_GT, tmp_GT, tmp_GT_2);
    ...
    // res = c0 / tmp_GT_3
    element_div(res, C.getC0()[c0], tmp_GT_3);
}
